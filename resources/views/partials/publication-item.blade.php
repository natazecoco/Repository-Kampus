<article class="group relative flex flex-col items-start gap-4 rounded-3xl bg-white p-5 sm:p-6 border border-slate-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] transition-all duration-300 hover:-translate-y-1.5 hover:border-gundar-primary/30 hover:shadow-[0_12px_40px_rgba(118,58,151,0.08)] sm:flex-row sm:gap-6">
    ...
    <div class="w-full shrink-0 sm:w-28 pt-1 text-left flex flex-row sm:flex-col items-center sm:items-start gap-3 sm:gap-0">
        ...
        <div class="flex sm:flex-col gap-2 mt-0 sm:mt-3 ml-auto sm:ml-0">
            <!-- Badge Metode Riset (Tanpa Emoji) -->
            @if($pub->research_method)
                <a href="{{ route('home', ['method' => $pub->research_method]) }}" class="inline-flex items-center gap-1 rounded-md bg-slate-50 px-2 py-1 text-[10px] font-bold text-slate-500 border border-slate-200 hover:bg-gundar-primary hover:text-white hover:border-gundar-primary transition-colors shadow-sm" title="Filter berdasarkan metode ini">
                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 11-3 0m3 0a1.5 1.5 0 10-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m-9.75 0h9.75" /></svg>
                    {{ $pub->research_method }}
                </a>
            @endif

            <!-- Badge Populer (Dengan Ikon Api SVG) -->
            @if(($pub->views_count ?? 0) >= 50 || $pub->files->sum('downloads_count') >= 20)
                <span class="inline-flex items-center gap-1 rounded-md bg-gradient-to-r from-amber-50 to-orange-50 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-amber-600 border border-amber-200/50 shadow-sm">
    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="mt-5 flex flex-wrap items-center justify-between gap-4 relative z-10 border-t border-slate-50 pt-4">
            ...
            <div class="flex flex-wrap items-center gap-4">
                ...
                @if($pub->topics->isNotEmpty())
                    <div class="flex gap-2">
                        @foreach($pub->topics->take(2) as $topic)
                            <a href="{{ route('topic.show', $topic->slug) }}" class="rounded-full bg-gundar-primary/5 px-2.5 py-1 text-[10px] font-bold text-gundar-primary hover:bg-gundar-primary hover:text-white transition-colors">
                                #{{ $topic->name }}
                            </a>
                        @endforeach
                    </div>
                ...
                @if($pub->files->count() > 0)
                    <div class="flex items-center gap-1 text-[11px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md">
            ...
            <!-- Statistik Mini (Views & Downloads dengan SVG) -->
            ...
            @else
                ...
                <form action="{{ route('bookmarks.toggle') }}" method="POST">
                    ...
                    <button type="submit" class="group relative z-30 flex items-center justify-center w-10 h-10 rounded-full transition-transform duration-300 hover:scale-110 active:scale-90" title="Simpan ke koleksi">
                        <!-- Latar belakang glow halus saat di-hover -->

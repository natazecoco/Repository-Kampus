<article class="group relative flex flex-col items-start gap-4 rounded-[32px] border border-white/60 bg-white/80 p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1.5 hover:border-gundar-primary/30 hover:shadow-[0_12px_40px_rgba(118,58,151,0.08)] sm:flex-row sm:gap-6 sm:p-6">
    <div class="flex w-full shrink-0 flex-row items-center gap-3 pt-1 text-left sm:w-28 sm:flex-col sm:items-start sm:gap-0">
        ...
        <div class="ml-auto flex gap-2 sm:ml-0 sm:mt-3 sm:flex-col">
            @if($pub->research_method)
                <a href="{{ route('home', ['method' => $pub->research_method]) }}" class="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50/50 px-2 py-1.5 text-[10px] font-bold text-slate-500 shadow-sm transition-colors hover:border-gundar-primary hover:bg-gundar-primary hover:text-white" title="Filter berdasarkan metode ini">
    ...
    <div class="min-w-0 flex-1">
        <h3 class="pr-10 text-xl font-black leading-snug text-gundar-dark transition group-hover:text-gundar-primary">
        ...
        <div class="relative z-10 mt-5 flex flex-wrap items-center justify-between gap-4 border-t border-slate-100/60 pt-4">
            <div class="flex flex-wrap items-center gap-4">
                @if($pub->topics->isNotEmpty())
                    <div class="flex flex-wrap gap-2">
                        @foreach($pub->topics->take(3) as $topic)
                            <a href="{{ route('home', ['search' => $topic->name]) }}" class="rounded-full border border-gundar-primary/10 bg-gundar-primary/5 px-3 py-1 text-[10px] font-bold text-gundar-primary transition-colors hover:border-gundar-primary hover:bg-gundar-primary hover:text-white">
                                #{{ $topic->name }}
                            </a>
                        @endforeach
                    </div>
